...
const UpdateSubCategoryComp = ({show,handleClose,token,currentData}) => {
  const [showToast, setShowToast] = useState(false);
  const [toastMessage, setToastMessage] = useState('');
  const [toastVariant, setToastVariant] = useState('');

  const category=currentData.CategoryName;
  const [CategoryName,setCategoryName]=useState(currentData.CategoryName);
  const [CategoryDisc,setCategoryDisc]=useState(currentData.CategoryDisc);

  // const CategoryName=UCategoryName.toUpperCase();
  const NewCategoryData={CategoryName,CategoryDisc};

  const update=async()=>{
    const response = await axios.put(`http://localhost:8000/api/v1/admin/updatesubcategory/${category}`,NewCategoryData,{
      headers:{
  ...
  return (
    ...
    <Form className="mb-3">
      <Form.Group className="mb-3">
        ...
        <Form.Control type="text" placeholder="Enter Category" value={CategoryName}/>
      </Form.Group>
      <Form.Group className="mb-3">
